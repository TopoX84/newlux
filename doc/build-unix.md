UNIX BUILD NOTES
====================
Some notes on how to build LUXCORE in Unix.

(For BSD specific instructions, see `build-*bsd.md` in this directory.)

Note
---------------------
Always use absolute paths to configure and compile LUXCORE and the dependencies.
...
memory available when compiling LUXCORE. On systems with less, gcc can be
...
To build LUXCORE without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
You need to build boost manually, and if it's not in standard library paths, you need to add `/path/to/boost/lib` into `LD_LIBRARY_PATH` env when building lux.
...
Security
--------
To help make your LUXCORE installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, LUXCORE should be built with a non-executable stack,
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, LUXCORE may be compiled in
...
This example lists the steps necessary to setup and build a command line only, non-wallet distribution of the latest changes on Arch Linux:
    ...
    git clone https://github.com/LUX-Core/newlux --recursive
    cd lux/
    ./autogen.sh
