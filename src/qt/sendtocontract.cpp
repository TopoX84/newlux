    ...
    m_results(1)
{
    ...
    ui->labelSenderAddress->setToolTip(tr("The LUX address that will be used as sender."));
...
void SendToContract::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->pushButtonSendToContract->setToolTip(tr("Creates a Partially Signed LUX Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    }
}
...
void SendToContract::on_sendToContractClicked()
{
    if(isDataValid())
    {
        ...
        if (bCreateUnsigned) {
            ...
            questionString.append(tr("Please, review your transaction proposal. This will produce a Partially Signed LUX Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
